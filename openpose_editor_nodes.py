...
class OpenposeEditorNode:
    ...
    def load_pose(self, show_body, show_face, show_hands, resolution_x, pose_marker_size, face_marker_size, hand_marker_size, hands_scale, body_scale, head_scale, overall_scale, scalelist_behavior, match_scalelist_method, only_scale_pose_index, POSE_JSON: str, POSE_KEYPOINT=None) -> tuple[OpenposeJSON]:
        '''
        Priority: POSE_JSON > POSE_KEYPOINT.

        When POSE_JSON is non-empty (written back by the editor after the user
        makes edits), it takes precedence over a connected POSE_KEYPOINT. Use
        the "Clear Edit" button on the node to discard edits and revert to the
        live POSE_KEYPOINT.
        '''
        if POSE_JSON:
        ...
        # No inputs — output a blank canvas
        W = 512
        H = 768
